...
{
	if (signal(sig,SIG_IGN)==BADSIG) return;
	switch(sig)
	{
//		case SIGPWR	: 
		case SIGHUP	: 
...
static void SetReXSig(int sig,void (*fcn)())
{
void (*ssr)();
...
//	SetUpMySignals();
	...
	if(argc>2) Main_Info=1;
	
	ob=NULL;
	CreateMainWindow();
	
	for(ow=0;((ow<255)&&(argv[1][ow]!='\0'));ow++) CommandLineBuffer[ow]=argv[1][ow];
	...
	if(ob!=NULL)
	{
		...
		{
			printf("\n\nCan not allocate next image structure... ?! whaaat?!!!");fflush(stdout);
			return;
		}
	...
	{
		printf("\nError creating Picture");fflush(stdout);
	}
	CleanUp();
