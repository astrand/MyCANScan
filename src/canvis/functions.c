...
char SearchTerm[16];
int SearchPos, SpeedPos = 0, DataOffset = 0x3FFF, DoubleBytes = 0, ShowSpeed = 1, ShowKeys =
    1, ShowTimes = 1, Show0 = 1, UseSigned = 0, Use12bitSigned = 0, NoSignumDraw = 0, LineType = 0;
...
int
CollectColors(char *BPTR, int TTX, int TTY, int NUMCO)
{
...
//                      if(b==0x3C8) SpeedPos=a;
    ...
    printf("\nFound %d unique IDs in %d messages...", a, NumberOfSamples);
    fflush(stdout);
#if 0
...
int
MiscellaneousKey(int modi, unsigned char key)
{
    int b;
