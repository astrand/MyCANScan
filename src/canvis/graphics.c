...
#if 0
 void	(*LFunc)();
 if((LFunc=(void(*)())LClientData)!=NULL) LFunc();
#endif
...
//	if(WorkImage!=NULL) XDestroyImage(WorkImage);
	if(WorkPixmap!=(Pixmap)0) XFreePixmap(WorkDisplay,WorkPixmap);
	if(WorkCursor!=0) XFreeCursor(WorkDisplay,WorkCursor);
	if(WorkPixmapGC!=NULL) XFreeGC(WorkDisplay,WorkPixmapGC);
	if(WorkWindowGC!=NULL) XFreeGC(WorkDisplay,WorkWindowGC);
	if((WorkWindow!=0)&&(WorkDisplay!=NULL)) XDestroyWindow(WorkDisplay,WorkWindow);
	if(WorkDisplay!=NULL) XCloseDisplay(WorkDisplay);
	if(ImageBuffer!=NULL) free((void*)ImageBuffer);
	if(WorkData!=NULL) free((void*)WorkData);
}
...
register unsigned char	Rv,Gv,Bv;
	...
	if(DrawTarget)
	{
		if((WBpy+(fh*zoom))>=DisplayHeight) tr=fsy+(DisplayHeight-WBpy);	// don't copy if out of screen (y>width)
		if((tx+(fw*zoom))>=DisplayWidth) tc-=(tx+(fw*zoom)-DisplayWidth);
	}
	else
	{
		if((WBpy+(fh*zoom))>=WorkHeight) tr=fsy+(WorkHeight-WBpy);	// don't copy if out of screen (y>width)
		if((tx+(fw*zoom))>=WorkWidth) tc-=(tx+(fw*zoom)-WorkWidth);
	}
			...
			if(fonts[b].x>=0)
			{
				...
				else
				{
					if((fonts[b].letter=='%')&&(!usebignums))
						...
						TransferFont(fonts[b].x,fonts[b].y,px,y,fonts[b].width,fonts[b].height,zoom);
				}
...
void TransferFontB(char *TB, int xsz, int ysz, int fsx, int fsy, int tx, int ty, int fw, int fh,int zoom)
{
register char	*FRPtr,ch;		// current row's string ptr
register int	cr=fsy,tr=fsy+fh;	// current / target row
register int	cc,sc=fsx,tc=fsx+fw;	// current / start / target column
...
register struct ImageBufferStructure	*IB;
	...
	if((WBpy+(fh*zoom))>=ysz) tr=fsy+(ysz-WBpy);	// don't copy if out of screen (y>width)
	if((tx+(fw*zoom))>=xsz) tc-=(tx+(fw*zoom)-xsz);
				...
				for(sz1=0;sz1<zoom;sz1++)
				{
					...
					if(WBPtr>=0)
					{
						for(cp=0;cp<FONTMAP_COLORS;cp++)
						{
							if(Font_Color[cp].letter==ch)
							{
								Rv=(unsigned char)ABS((int)IB[WBPtr].R-(int)Font_Color[cp].Color);
								Gv=(unsigned char)ABS((int)IB[WBPtr].G-(int)Font_Color[cp].Color);
								Bv=(unsigned char)ABS((int)IB[WBPtr].B-(int)Font_Color[cp].Color);
								IB[WBPtr].R=Rv;
								IB[WBPtr].G=Gv;
								IB[WBPtr].B=Bv;
							}
...
void PutMyStringB(char* TB,int xsz, int ysz, char *Text,int x, int y, int usebignums, int zoom)
{
int	a=0,b,px=x,notfound;
	...
	while(Text[a]!='\0')
	{
		...
		if(usebignums)
		{
			...
			if(bignumbers[b].x>=0)
			{
				TransferFontB(TB,xsz,ysz,bignumbers[b].x,bignumbers[b].y,px,y-2,bignumbers[b].width,bignumbers[b].height,zoom);
				px+=bignumbers[b].width*zoom;
				notfound=0;
			}
		}
		if(notfound)
		{
			...
			{
				if((fonts[b].letter>='0')&&(fonts[b].letter<='9'))
					TransferFontB(TB,xsz,ysz,fonts[b].x,fonts[b].y,px,y-1,fonts[b].width,fonts[b].height,zoom);
				else
				{
					if((fonts[b].letter=='%')&&(!usebignums))
						TransferFontB(TB,xsz,ysz,fonts[b].x,fonts[b].y,px,y-2,fonts[b].width,fonts[b].height,zoom);
					else
						TransferFontB(TB,xsz,ysz,fonts[b].x,fonts[b].y,px,y,fonts[b].width,fonts[b].height,zoom);
				}
				px+=fonts[b].width*zoom;
			}
		}
...
void GR_Refresh(void)
{
	XSync(WorkDisplay,False);
}

void GR_GetPixel(int x, int y, unsigned char *GRv_BckR, unsigned char *GRv_BckG, unsigned char *GRv_BckB)
{
register int	c;
...
void GR_PutPixel(int x, int y, unsigned char GRv_BckR, unsigned char GRv_BckG, unsigned char GRv_BckB)
{
register int	c;
...
void GR_DrawLine(int gx1,int gy1,int gx2,int gy2,int mode, unsigned char GRv_FwdR, unsigned char GRv_FwdG, unsigned char GRv_FwdB)	// mode : 0-put 1-antialias 2-negate
{
...
printf("\nGR_DrawLine(%d,%d,%d,%d, %d, %03d,%03d,%03d)",gx1,gy1,gx2,gy2,mode,GRv_FwdR,GRv_FwdG,GRv_FwdB);fflush(stdout);
#endif
	...
	if(y1>y2) ybgr=0; else ybgr=1;

	if(ABS((x2-x1))>ABS((y2-y1)))
	{
		xs=((x2>=x1)?(1.0):(-1.0));
		ys=(float)(y2-y1)/(float)(ABS((x2-x1)));
		nofs=ABS((x2-x1));hnt=0;
	}
	else
	{
		xs=(float)(x2-x1)/(float)(ABS((y2-y1)));
		ys=((y2>=y1)?(1.0):(-1.0));
		nofs=ABS((y2-y1));hnt=1;
	}
	x=(float)x1,y=(float)y1;a=0;
	...
	switch(mode)
	{
			...
			do
			{
				if(!hnt)	// step in x
				{
					if(CHNK(y)>=(float)0.5)
					{
						...
						mr=(unsigned char)((((float)GRv_BckR-(float)GRv_FwdR)*(1-CHNK(y)))+GRv_FwdR);
						mg=(unsigned char)((((float)GRv_BckG-(float)GRv_FwdG)*(1-CHNK(y)))+GRv_FwdG);
						mb=(unsigned char)((((float)GRv_BckB-(float)GRv_FwdB)*(1-CHNK(y)))+GRv_FwdB);
						GR_PutPixel((int)x,(int)(y+1),mr,mg,mb);
						mr=(unsigned char)((((float)GRv_BckR-(float)GRv_FwdR)*CHNK(y))+GRv_FwdR);
						mg=(unsigned char)((((float)GRv_BckG-(float)GRv_FwdG)*CHNK(y))+GRv_FwdG);
						mb=(unsigned char)((((float)GRv_BckB-(float)GRv_FwdB)*CHNK(y))+GRv_FwdB);
						if(ybgr)
				...
				else
				{
					if(CHNK(x)>=(float)0.5)
					{
						mr=(unsigned char)((((float)GRv_BckR-(float)GRv_FwdR)*(1-CHNK(x)))+GRv_FwdR);
						mg=(unsigned char)((((float)GRv_BckG-(float)GRv_FwdG)*(1-CHNK(x)))+GRv_FwdG);
						mb=(unsigned char)((((float)GRv_BckB-(float)GRv_FwdB)*(1-CHNK(x)))+GRv_FwdB);
						GR_PutPixel((int)(x+((xs<0)?(-1):(1))),(int)y,mr,mg,mb);
						mr=(unsigned char)((((float)GRv_BckR-(float)GRv_FwdR)*CHNK(x))+GRv_FwdR);
						mg=(unsigned char)((((float)GRv_BckG-(float)GRv_FwdG)*CHNK(x))+GRv_FwdG);
						mb=(unsigned char)((((float)GRv_BckB-(float)GRv_FwdB)*CHNK(x))+GRv_FwdB);
						GR_PutPixel((int)x,(int)y,mr,mg,mb);
					}
					else
					{
		...
		case 2 :
			do
			{
				...
				GR_PutPixel((int)x,(int)y,(unsigned char)(255-GRv_BckR),(unsigned char)(255-GRv_BckG),(unsigned char)(255-GRv_BckB));
				x+=xs;y+=ys;++a;
			}while(a<=nofs);
		break;
	}
...
void GR_DrawRectangle(int gx1,int gy1,int gx2,int gy2,int mode, unsigned char GRv_FwdR, unsigned char GRv_FwdG, unsigned char GRv_FwdB)	// mode : 0-put 1-antialias 2-negate
{

#ifdef DEBUG_GRAPHOPS
printf("\nGR_DrawRectangle(%d,%d,%d,%d, %d, %03d,%03d,%03d)",gx1,gy1,gx2,gy2,mode,GRv_FwdR,GRv_FwdG,GRv_FwdB);fflush(stdout);
#endif
...
void GR_DrawFillRectangle(int gx1,int gy1,int gx2,int gy2,int mode, unsigned char GRv_FwdR, unsigned char GRv_FwdG, unsigned char GRv_FwdB)	// mode : 0-put 1-antialias 2-negate
{
register int	ty;
...
void ClearWorkBuffer(void)
{
register int	a,b,c;

#ifdef DEBUG_BUFFERS
printf("\nClearWorkBuffer");fflush(stdout);
#endif
...
void ReDrawWorkBuffer(int x, int y, int w, int h)
{
...
#ifdef DEBUG_BUFFERS
printf("\nClearDisplayBuffer (%x)",ImageBuffer);fflush(stdout);
#endif
...
void CopyWorkBufferToScreen(void)
{
...
#ifdef DEBUG_BUFFERS
printf("\nCopyWorkBufferToScreen");fflush(stdout);
#endif
	...
	switch(WorkBitsPerRGB)
	{
		case 8 :
			for(b=0;b<WorkHeight;b++)
				...
				{
					c=a+b*DisplayWidth;
					TmpChrWrk[c]=(char)((ImageBuffer[c].R>>WorkRedNumberOfShifts)<<WorkRedNumberOfUpShifts)|((ImageBuffer[c].G>>WorkGreenNumberOfShifts)<<WorkGreenNumberOfUpShifts)|((ImageBuffer[c].B>>WorkBlueNumberOfShifts)<<WorkBlueNumberOfUpShifts);
				}
		break;
		case 16 :
			for(b=0;b<WorkHeight;b++)
...
#if 0
if(b==0) printf("%d=[%04x:%04x:%04x = %04x]  ",a,((ImageBuffer[c].R>>WorkRedNumberOfShifts)<<WorkRedNumberOfUpShifts),((ImageBuffer[c].G>>WorkGreenNumberOfShifts)<<WorkGreenNumberOfUpShifts),((ImageBuffer[c].B>>WorkBlueNumberOfShifts)<<WorkBlueNumberOfUpShifts),TmpWrk[c]);
fflush(stdout);
#endif
...
#ifdef DEBUG_BUFFERS
printf("\nCopyDisplayBufferToScreen (%d,%d,%d,%d %d)",x,y,w,h,WorkBitsPerRGB);fflush(stdout);
#endif

	wx=0,wy=0;ww=DisplayWidth-1;wh=DisplayHeight;
	...
	if(wh>=DisplayHeight) wh=DisplayHeight-1;
	if(ww>=DisplayWidth) ww=DisplayWidth-1;
...
#ifdef DEBUG_BUFFERS
printf(" -> %d:%d %dx%d ok.",wx,wy,ww,wh);fflush(stdout);
#endif

}


void CopyDisplayBufferRectangleToScreen(int x1,int y1,int x2, int y2)
{
...
#ifdef DEBUG_BUFFERS
printf("\nCopyDisplayBufferRectangleToScreen (%d,%d,%d,%d)",x1,y1,x2,y2);fflush(stdout);
#endif
...
void	CopyImageToWorkBuffer(int ctr)
{
...
#ifdef DEBUG_BUFFERS
printf("\nCopyImageToWorkBuffer(%d) = 0x%0x",ctr,MyImages[ctr].Buffer);fflush(stdout);
#endif

	if(MyImages[ctr].Buffer==NULL) return;
	...
	if((FadeFactor=MyImages[ctr].FadeFactor)>(float)0.95) nofade=1;
	...
	if((poslsx=(int)((float)(0-MyImages[ctr].X)/ZoomFactor))<0) poslsx=0;
	if((poslsy=(int)((float)(0-MyImages[ctr].Y)/ZoomFactor))<0) poslsy=0;

	if((poslex=(int)((float)(ww-MyImages[ctr].X)/ZoomFactor)+1)>MyImages[ctr].Width) poslex=MyImages[ctr].Width;
	if((posley=(int)((float)(wh-MyImages[ctr].Y)/ZoomFactor)+1)>MyImages[ctr].Height) posley=MyImages[ctr].Height;

	TmpCPtr=MyImages[ctr].Buffer;

if(Verbose)
{
	printf("\n%d. image = %d:%d %dx%d [%d] Zoom=%1.4f [%d]  ssx=%d ssy=%d",ctr,MyImages[ctr].X,MyImages[ctr].Y,MyImages[ctr].Width,MyImages[ctr].Height,MyImages[ctr].NumOfComponent,ZoomFactor,zct,ssx,ssy);fflush(stdout);
	printf("\n\t ppos = %d:%d - %d:%d spos = %d:%d - %d:%d  [%08x -> %08x]\n",ppsx,ppsy,ppex,ppey,poslsx,poslsy,poslex,posley,TmpCPtr,ImageBuffer);fflush(stdout);
}
...
//printf("\n %d:%d [%d:%d] => [%d:%d]",Sbp,Tbp,wpx,wpy,iwsx+wx,(int)wsy+wy);fflush(stdout);
						...
						else
						{
							...
							if(masko&M_BLUE)
								...
								tb=0.0f;
							ImageBuffer[Tbp].R=(unsigned char)((tr*(1.0f-FadeFactor))+((float)((unsigned char)TmpCPtr[Sbp])*FadeFactor));
							ImageBuffer[Tbp].G=(unsigned char)((tg*(1.0f-FadeFactor))+((float)((unsigned char)TmpCPtr[Sbp+1])*FadeFactor));
							ImageBuffer[Tbp].B=(unsigned char)((tb*(1.0f-FadeFactor))+((float)((unsigned char)TmpCPtr[Sbp+2])*FadeFactor));
...
void	CopyImagesToWorkBuffer(void)
{
int	ctr;
...
if(Verbose)
{
	printf("\nWorkArea = %dx%d, Number of Images = %d",WorkWidth,WorkHeight,NumberOfImages);fflush(stdout);
}

	for(ctr=0;ctr<NumberOfImages;ctr++)
	{
		if(ctr!=SelectedImage) CopyImageToWorkBuffer(ctr);
	}

	if(SelectedImage!=-1)
	{
		CopyImageToWorkBuffer(SelectedImage);
		GR_DrawRectangle(MyImages[SelectedImage].X-1,MyImages[SelectedImage].Y-1,(int)((float)MyImages[SelectedImage].Width*MyImages[SelectedImage].ZoomFactor)+MyImages[SelectedImage].X,(int)((float)MyImages[SelectedImage].Height*MyImages[SelectedImage].ZoomFactor)+MyImages[SelectedImage].Y,0,255,0,0);
	}
}
...
int CreateButton(char* name,void(*ClientFunction)(void*),void *ClientData,unsigned char AccChr,int(*ClientKeyFunction)(int,char),unsigned char ImageManipulate,int x, int y, int w, int h,int(*ButtonFunction)(int,int,char,unsigned char))
{
ButtonStructure *TmpButton=NULL;
register int	a,b=-1;

	if((TmpButton=(ButtonStructure *)realloc((void*)Buttons,(size_t)(sizeof(struct ButtonStructure)*(NumberOfButtons+1))))==NULL)
	{
		printf("\nError with Adding button!!!");fflush(stdout);
		return(1);
	}
	Buttons=TmpButton;
	...
	if(w!=0)
		...
		Buttons[NumberOfButtons].w=GetMyStringLength(Buttons[NumberOfButtons].name,0,1)+(DEF_BUT_HOR_PAD<<1);
	if(h!=0)
...
register int	a,b;
	...
	for(a=0;a<NumberOfButtons;a++)
	{
		if(ActiveButton==a)
		{
			GR_DrawRectangle(Buttons[a].x,Buttons[a].y,Buttons[a].x+Buttons[a].w,Buttons[a].y+Buttons[a].h,1,Bts_R,Bts_G,Bts_B);
			GR_DrawRectangle(Buttons[a].x+1,Buttons[a].y+1,Buttons[a].x+Buttons[a].w-1,Buttons[a].y+Buttons[a].h-1,1,Bts2_R,Bts2_G,Bts2_B);
		}
		else
		{
			GR_DrawRectangle(Buttons[a].x,Buttons[a].y,Buttons[a].x+Buttons[a].w,Buttons[a].y+Buttons[a].h,1,Bt_R,Bt_G,Bt_B);
			GR_DrawRectangle(Buttons[a].x+1,Buttons[a].y+1,Buttons[a].x+Buttons[a].w-1,Buttons[a].y+Buttons[a].h-1,1,Bt_R,Bt_G,Bt_B);
		}
		GR_DrawFillRectangle(Buttons[a].x+2, Buttons[a].y+2, Buttons[a].x+Buttons[a].w-2, Buttons[a].y+Buttons[a].h-2, 0, Bt_BgR, Bt_BgG, Bt_BgB);
		b=GetMyStringLength(Buttons[a].name,0,1);
		PutMyString(Buttons[a].name,Buttons[a].x+DEF_BUT_HOR_PAD,Buttons[a].y+DEF_BUT_VER_PAD+1,0,1);
	}
}

int HandleButtonPush(int x, int y)
{
register int	a,w,h;
	...
	for(a=0;a<NumberOfButtons;a++)
	{
		if((x>=Buttons[a].x)&&(y>=Buttons[a].y)&&((Buttons[a].x+Buttons[a].w)>=x)&&((Buttons[a].y+Buttons[a].h)>=y))
		{
			...
			else
			{
				if((SelectedImage!=-1)||(!Buttons[a].ImageManipulate))
				{
					if(ActiveButton!=-1)
					{
						GR_DrawRectangle(Buttons[ActiveButton].x,Buttons[ActiveButton].y,Buttons[ActiveButton].x+Buttons[ActiveButton].w,Buttons[ActiveButton].y+Buttons[ActiveButton].h,1,Bt_R,Bt_G,Bt_B);
						GR_DrawRectangle(Buttons[ActiveButton].x+1,Buttons[ActiveButton].y+1,Buttons[ActiveButton].x+Buttons[ActiveButton].w-1,Buttons[ActiveButton].y+Buttons[ActiveButton].h-1,1,Bt_R,Bt_G,Bt_B);
					}
					GR_DrawRectangle(Buttons[a].x,Buttons[a].y,Buttons[a].x+Buttons[a].w,Buttons[a].y+Buttons[a].h,1,Bts_R,Bts_G,Bts_B);
					GR_DrawRectangle(Buttons[a].x+1,Buttons[a].y+1,Buttons[a].x+Buttons[a].w-1,Buttons[a].y+Buttons[a].h-1,1,Bts2_R,Bts2_G,Bts2_B);
					ActiveButton=a;
			...
			CopyDisplayBufferToScreen(Buttons[ActiveButton].x-2,Buttons[ActiveButton].y-2,Buttons[ActiveButton].w+2,Buttons[ActiveButton].h+2);
			return(-1);
		}
	}
	if(y>WorkHeight) return(-1);		// Menu area
...
void HandleSystemKeys(int kmodifier,unsigned char Key)
{
int	a,RetSize=-1;
char*	ob;
int	ow,oh,obs;
...
//printf("\n%d [%d]",Key,kmodifier);fflush(stdout);
	...
	switch(Key)
	{
		case SK_F1 :
			if(SelectedImage!=-1)
			{
				MyImages[SelectedImage].X-=(int)((float)MyImages[SelectedImage].HotPosX*MyImages[SelectedImage].ZoomFactor);
				MyImages[SelectedImage].Y-=(int)((float)MyImages[SelectedImage].HotPosY*MyImages[SelectedImage].ZoomFactor);
				if((MyImages[SelectedImage].ZoomFactor*=(float)2.0)>MAX_ZOOMFACTOR) MyImages[SelectedImage].ZoomFactor=MAX_ZOOMFACTOR;
				ClearWorkBuffer();
		...
		case SK_F2 :
			if(SelectedImage!=-1)
			{
				if((MyImages[SelectedImage].ZoomFactor*=(float)0.5)<MIN_ZOOMFACTOR) MyImages[SelectedImage].ZoomFactor=MIN_ZOOMFACTOR;
				MyImages[SelectedImage].X+=(int)((float)MyImages[SelectedImage].HotPosX*MyImages[SelectedImage].ZoomFactor);
				MyImages[SelectedImage].Y+=(int)((float)MyImages[SelectedImage].HotPosY*MyImages[SelectedImage].ZoomFactor);
				ClearWorkBuffer();
				CopyImagesToWorkBuffer();
		...
		case SK_END :
			if(SelectedImage!=-1)
			{
				MyImages[SelectedImage].X=(int)(((float)DisplayWidth-((float)MyImages[SelectedImage].Width*MyImages[SelectedImage].ZoomFactor))*0.5f);
				MyImages[SelectedImage].Y=(int)(((float)DisplayHeight-((float)MyImages[SelectedImage].Height*MyImages[SelectedImage].ZoomFactor))*0.5f);
				ButtonPressedImageX=0;
				ButtonPressedImageY=0;
				MyImages[SelectedImage].HotPosX=0;
				MyImages[SelectedImage].HotPosY=0;
...
void HandleButtonKey(int kmodifier,unsigned char Key)
{
...
//printf("\nHandleButtonKey : '%c' = %d  (mod = %d) Activebutton=%d",Key,Key,kmodifier,ActiveButton);fflush(stdout);
	...
	if(ActiveButton==-1)
	{
		for(a=0;a<NumberOfButtons;a++)
		{
			if(Key==Buttons[a].PushChar)
			{
				...
				CopyDisplayBufferToScreen(Buttons[a].x-1,Buttons[a].y-1,Buttons[a].w+2,Buttons[a].h+2);
				if(Buttons[a].PushFunction!=NULL)
					Buttons[a].PushFunction(Buttons[a].PushClientData);
				return;
			}
	...
	if(ret==0) HandleSystemKeys(kmodifier,Key);
}
...
int	a=NumberOfImages;
	...
	++NumberOfImages;
	if((MyImages=(struct MyImagesStructure*)realloc((void*)MyImages,(size_t)((sizeof(struct MyImagesStructure))*NumberOfImages)))==NULL)
	{
		printf("Error ReAllocating MyImages structure.");fflush(stdout);
		return(-1);
	}

#ifdef ALLOC_TRACK
printf("\nA(r) : graphics : InitializeNextImageStructure : MyImages %08x (%d)",MyImages,((sizeof(struct MyImagesStructure))*NumberOfImages));fflush(stdout);
#endif
...
int UpdateImageToOriginal(int a)
{
	if(a>=NumberOfImages) return(-1);
	...
	if(MyImages[a].Buffer==NULL) return(-1);
return(0);
}

int MoveImageModifiedToOriginal(int a)
{
	if(a>=NumberOfImages) return(-1);

	if(MyImages[a].OriginalBuffer!=NULL) free((void*)MyImages[a].OriginalBuffer);
...
int UpdateImageToModified(int a)
{
	if(a>=NumberOfImages) return;
	if(MyImages[a].ModifiedBuffer==NULL) return(-1);
...
// First Let's see, what visuals does the system support...
	...
	if(Main_Info)
	{
		...
		for(b=0;b<NumVis;b++)
		{
			printf("\n\t%d. :\n\t\tVisual=%0x\n\t\tVidusalID=%d\n\t\tScreen=%d\n\t\tDepth=%d\n\t\tClass=%d\n\t\tMasks= %04x : %04x : %04x\n\t\tColormap Size=%d\n\t\tBits per RGB=%d",b,WorkWithThisVisInfo->visual,WorkWithThisVisInfo->visualid,MyVisInfo->screen,WorkWithThisVisInfo->depth,MyVisInfo->class,WorkWithThisVisInfo->screen,WorkWithThisVisInfo->red_mask,WorkWithThisVisInfo->green_mask,WorkWithThisVisInfo->blue_mask,WorkWithThisVisInfo->colormap_size,WorkWithThisVisInfo->bits_per_rgb);fflush(stdout);
			WorkWithThisVisInfo++;
		}
		printf("\n\n");fflush(stdout);
	}
	...
	if((MyPFVInfo=XListPixmapFormats(WorkDisplay,&NumRet))!=NULL)
	{
		...
		for(a=0;a<NumRet;a++)
		{
			if(Main_Info)
			{
				printf("\n\t%d. Depth=%d  Bits_per_Pixel=%d  Scanline_pad=%d)",a,WorkWithThisPFV->depth,WorkWithThisPFV->bits_per_pixel,WorkWithThisPFV->scanline_pad);
				fflush(stdout);
			}
			...
			for(b=0;b<NumVis;b++)
			{
				...
				{
					if(Main_Info) {printf(" -> MATCH!");fflush(stdout);}
	...
	if(Main_Info)
	{
		...
		printf("\n WorkRedMask    = %04x (%d) (%d)",WorkRedMask,WorkRedNumberOfShifts,WorkRedNumberOfUpShifts);
		printf("\n WorkGreenMask  = %04x (%d) (%d)",WorkGreenMask,WorkGreenNumberOfShifts,WorkGreenNumberOfUpShifts);
		printf("\n WorkBlueMask   = %04x (%d) (%d)",WorkBlueMask,WorkBlueNumberOfShifts,WorkBlueNumberOfUpShifts);
		printf("\n WorkDepthShift = %d (%d)",(WorkDepth>>3),sizeof(unsigned short));fflush(stdout);
		fflush(stdout);
	}
...
//	wd=WorkDepth;
	...
	{
		if(Main_Info) {printf("\nCan not allocate %d bytes for WorkData.",((WorkDepth>>3)*DisplayWidth*DisplayHeight));fflush(stdout);}
		return(-4);
	}

#ifdef ALLOC_TRACK
printf("\nA : graphics : GetHighestVisualPixmapCombination : WorkData %08x (%d)",WorkData,((wd>>3)*DisplayWidth*DisplayHeight));fflush(stdout);
#endif

	if((ImageBuffer=(struct ImageBufferStructure *)malloc(sizeof(struct ImageBufferStructure)*DisplayWidth*DisplayHeight))==NULL)
	{
		if(Main_Info) {printf("\nCan not allocate %d bytes for ImageBuffer.",(3*DisplayWidth*DisplayHeight));fflush(stdout);}
		return(-4);
	}

#ifdef ALLOC_TRACK
printf("\nA : graphics : GetHighestVisualPixmapCombination : ImageBuffer %08x (%d)",ImageBuffer,(sizeof(struct ImageBufferStructure)*DisplayWidth*DisplayHeight));fflush(stdout);
#endif
...
if(Main_Info) {printf("->Ok.");fflush(stdout);}
...
void	QuitApplication(void* Data)
{
	ProcessGo=0;
}
...
if(Main_Info) {printf("\nXCreateWindow");fflush(stdout);}
	...
	if((WorkWindow=XCreateWindow(WorkDisplay,RootWindow(WorkDisplay,WorkScreen),WorkPosX,WorkPosY,DisplayWidth,DisplayHeight,2,WorkDepth,InputOutput,WorkVisual,0,NULL))==0)
	{
...
if(Main_Info) {printf("->Ok.");fflush(stdout);}
	...
	xpos=30;CreateButton("Quit",QuitApplication,NULL,(unsigned char)'q',NULL,0,DisplayWidth-xpos,DisplayHeight-21,0,0,NULL);
	xpos+=2;
	xpos+=38;CreateButton("Save",SavePicture,NULL,(unsigned char)'s',NULL,1,DisplayWidth-xpos,DisplayHeight-21,0,0,NULL);
	xpos+=120;CreateButton("Process CAN Data",MiscellaneousButton,NULL,(unsigned char)' ',MiscellaneousKey,1,DisplayWidth-xpos,DisplayHeight-21,0,0,NULL);

	XMoveWindow(WorkDisplay,WorkWindow,WorkPosX,WorkPosY);
	XSelectInput(WorkDisplay,WorkWindow,KeyPressMask|KeyReleaseMask|ButtonPressMask|ButtonMotionMask|ButtonReleaseMask|PointerMotionMask|ExposureMask|StructureNotifyMask);
	XMapWindow(WorkDisplay,WorkWindow);

return(0);
}

int ReconfigureWindow(int width, int height)
{
XGCValues	mygcvalues;

if(Main_Info) {printf("Reconfiguring to %dx%d...",width,height);fflush(stdout);}
	...
	if((WorkData=(char*)realloc((void*)WorkData,(size_t)((WorkDepth>>3)*DisplayWidth*DisplayHeight)))==NULL)
	{
		printf("\nCan not reallocate %d bytes for WorkData.",((WorkDepth>>3)*DisplayWidth*DisplayHeight));fflush(stdout);
		return(1);
	}

	if((ImageBuffer=(struct ImageBufferStructure *)realloc((void*)ImageBuffer,(size_t)(sizeof(struct ImageBufferStructure)*DisplayWidth*DisplayHeight)))==NULL)
	{
		printf("\nCan not reallocate %d bytes for ImageBuffer.",(3*DisplayWidth*DisplayHeight));fflush(stdout);
		return(1);
	}
...
printf("\n\nEntering MainLoop");fflush(stdout);
#endif
...
#ifdef DEBUG_MESSAGES
printf("\n\nExpose %08x (%08x) %d:%d %dx%d",report.xexpose.window,WorkWindow,report.xexpose.x,report.xexpose.y,report.xexpose.width,report.xexpose.height);fflush(stdout);
#endif
...
#ifdef DEBUG_MESSAGES
printf("\n\nConfigureNotify %08x (%08x) %d:%d %dx%d",report.xconfigure.window,WorkWindow,report.xconfigure.x,report.xconfigure.y,report.xconfigure.width,report.xconfigure.height);fflush(stdout);
#endif

			if(WorkWindowInitialized)
			{
				if((report.xconfigure.width!=DisplayWidth)||(report.xconfigure.height!=DisplayHeight))
				{
			...
			if(report.xkey.window==WorkWindow)
			{
				switch((int)XLookupKeysym((XKeyEvent*)&report,0))
				{
					case XK_Shift_L :
						...
						if(!(InternalKeyModifiers&2)) KeyModifiers&=((unsigned char)255-(KM_SHIFT));
					break;
					case XK_Shift_R :
						InternalKeyModifiers&=((unsigned int)0xFFFF-2);
						if(!(InternalKeyModifiers&1)) KeyModifiers&=((unsigned char)255-(KM_SHIFT));
					break;
					case XK_Control_L :
						InternalKeyModifiers&=((unsigned int)0xFFFF-4);
						if(!(InternalKeyModifiers&8)) KeyModifiers&=((unsigned char)255-(KM_CTRL));
					break;
					case XK_Control_R :
						InternalKeyModifiers&=((unsigned int)0xFFFF-8);
						if(!(InternalKeyModifiers&4)) KeyModifiers&=((unsigned char)255-(KM_CTRL));
					break;
					case XK_Alt_L :
						InternalKeyModifiers&=((unsigned int)0xFFFF-16);
						if(!(InternalKeyModifiers&32)) KeyModifiers&=((unsigned char)255-(KM_ALT));
					break;
					case XK_Alt_R :
						InternalKeyModifiers&=((unsigned int)0xFFFF-32);
						if(!(InternalKeyModifiers&16)) KeyModifiers&=((unsigned char)255-(KM_ALT));
					break;
				}
				if(!XLookupString((XKeyEvent*)&report,(char*)buffer,bufsize,&keysym,&compose)) buffer[0]=0;
				...
				if(buffer[0]=='z')		// distance
				{
					if((SelectedImage>=0)&&(MyImages[SelectedImage].HotPosX>=0))
					{
						...
						{
							fv=((float)ZPressX-(float)MyImages[SelectedImage].HotPosX)*((float)ZPressX-(float)MyImages[SelectedImage].HotPosX);
							fv+=((float)ZPressY-(float)MyImages[SelectedImage].HotPosY)*((float)ZPressY-(float)MyImages[SelectedImage].HotPosY);
							fv=sqrt(fv);

printf("\nDistance %d:%d -> %d:%d = %1.2f  [%d:%d]",ZPressX,ZPressY,MyImages[SelectedImage].HotPosX,MyImages[SelectedImage].HotPosY,fv,ABS((MyImages[SelectedImage].HotPosX-ZPressX)),ABS((MyImages[SelectedImage].HotPosY-ZPressY)));fflush(stdout);
...
#ifdef DEBUG_MESSAGES
printf("\nKeyRelease = %08x (%08x)[%d] = %d %04x",report.xkey.window,WorkWindow,KeyModifiers,(int)XLookupKeysym((XKeyEvent*)&report,0),(int)XLookupKeysym((XKeyEvent*)&report,0));fflush(stdout);
#endif
...
#ifdef DEBUG_MESSAGES
printf("\nKeyPress = %08x (%08x) [%d] = %d %04x => '%c' '%d'",report.xkey.window,WorkWindow,KeyModifiers,(int)XLookupKeysym((XKeyEvent*)&report,0),(int)XLookupKeysym((XKeyEvent*)&report,0),buffer[0],(unsigned char)buffer[0]);fflush(stdout);
#endif
...
// printf("\n%d:%d [%d]",report.xmotion.x,report.xmotion.y,ButtonPressed);fflush(stdout);
			...
			switch(ButtonPressed)
			{
				case 0 :
					if(SelectedImage!=-1)
					{
						a=((float)(report.xmotion.x-MyImages[SelectedImage].X)/MyImages[SelectedImage].ZoomFactor);
						b=((float)(report.xmotion.y-MyImages[SelectedImage].Y)/MyImages[SelectedImage].ZoomFactor);
						if((a<0)||(a>=MyImages[SelectedImage].Width)) a=-1;
						if((b<0)||(b>=MyImages[SelectedImage].Height)) b=-1;
						if(a==-1) b=-1;
						else
							if(b==-1) a=-1;
					...
					if(SelectedImage!=-1)
					{
						if((KeyModifiers==2)&&(Buttons[ActiveButton].ButtonFunction!=NULL))
						{
							...
							if(a==-1) b=-1;
								...
								if(b==-1) a=-1;
							if((MyImages[SelectedImage].HotPosX!=a)||(MyImages[SelectedImage].HotPosY!=b))
							{
								MyImages[SelectedImage].HotPosX=a;
								MyImages[SelectedImage].HotPosY=b;
								Buttons[ActiveButton].ButtonFunction((int)a,(int)b,(char)(report.xbutton.button),KeyModifiers);
							}
						}
						else
						{
							...
							MyImages[SelectedImage].X=ButtonPressedImageX+(int)(report.xmotion.x)-ButtonPressedX;
							MyImages[SelectedImage].Y=ButtonPressedImageY+(int)(report.xmotion.y)-ButtonPressedY;
/*
	...
	case	ButtonPress:
			...
			if((ActiveButton!=-1)&&(SelectedImage!=-1))
			{
				if(Buttons[ActiveButton].ButtonFunction!=NULL)
				{
					...
					MyImages[SelectedImage].HotPosX=a;
					MyImages[SelectedImage].HotPosY=b;
					if(Buttons[ActiveButton].ButtonFunction((int)a,(int)b,(char)(report.xbutton.button),KeyModifiers)) h=0;
				}
			}
			...
			if(h)	// Was not processed
			{
				if(ButtonPressed==1)
				{
					...
					{
						if(SelectedImage!=a) PreviousSelectedImage=SelectedImage;
						SelectedImage=a;
						ButtonPressedImageX=MyImages[SelectedImage].X;
						ButtonPressedImageY=MyImages[SelectedImage].Y;
						a=((float)(report.xbutton.x-MyImages[SelectedImage].X)/MyImages[SelectedImage].ZoomFactor);
						b=((float)(report.xbutton.y-MyImages[SelectedImage].Y)/MyImages[SelectedImage].ZoomFactor);
						if((a<0)||(a>=MyImages[SelectedImage].Width)) a=-1;
						if((b<0)||(b>=MyImages[SelectedImage].Height)) b=-1;
						if(a==-1) b=-1;
						else
...
#ifdef DEBUG_MESSAGES
printf("\n%d %d [%d / %d]",(unsigned int)(report.xbutton.x),(unsigned int)(report.xbutton.y),(unsigned char)(report.xbutton.button),ButtonPressed);fflush(stdout);
#endif
		break;
		case	ButtonRelease:
			...
			if((report.xbutton.x>(DisplayWidth-10))&&(report.xbutton.y>(DisplayHeight-10))) return;
