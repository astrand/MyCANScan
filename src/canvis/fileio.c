...
typedef struct ColorStructure
{
	...
	float		MinDist;
	int		MinIndex;
}ColorStructure;extern ColorStructure	*MyClrStr;
...
int SavePNGFilePalette(char *FileName, char *SrcBuffer, int Width, int Height, int NumOfComponent, int NumClrs)
{
FILE		*fp=NULL;
...
png_color	pngcolorpalette[256];
	...
	if((FileName==NULL)||(SrcBuffer==NULL)||(NumOfComponent<1)||(Width<1)||(Height<1)) return(0);

	if((fp=fopen(FileName,"w"))==NULL)
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_PALETTE : Can not open '%s' for writing...",FileName);fflush(stdout);
#endif
		return(0);
	}
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_PALETTE : Can not allocate Write_Struct...");fflush(stdout);
#endif
		fclose(fp);
		return(0);
	}
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_PALETTE : Can not allocate Info_Struct...");fflush(stdout);
#endif
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_PALETTE : Can not allocate linebuffer... (%d bytes)",(int)bpl);fflush(stdout);
#endif
	...
	for(y=0,dbp=0;y<height;y++,dbp+=(width*3))
	{
		...
		for(x=0;x<width;x++)
		{
			...
			if(a<(NumClrs+10)) *(tmplbptr++)=(unsigned char)a;
		}
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE : Can not open for read-back '%s'.",FileName);fflush(stdout);
#endif
		return(1);
	}
...
int SavePNGFileRGB(char *FileName, char *SrcBuffer, int Width, int Height, int NumOfComponent)
{
FILE		*fp=NULL;
register int	dbp,tmpdbp,x,y,a;
png_structp	png_ptr;
...
char*		lbfptrs[1];
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_RGB : Can not open '%s' for writing...",FileName);fflush(stdout);
#endif
		return(0);
	}
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_RGB : Can not allocate Write_Struct...");fflush(stdout);
#endif
		fclose(fp);
		return(0);
	}
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_RGB : Can not allocate Info_Struct...");fflush(stdout);
#endif
		png_destroy_write_struct(&png_ptr,(png_infopp)NULL);
		fclose(fp);
		return(0);
	...
	if(depth>8) png_set_packswap(png_ptr);
#endif // BYTE_LITTLE_ENDIAN
	if((lbf=(char*)malloc((size_t)bpl))==NULL)
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_RGB : Can not allocate linebuffer... (%d bytes)",(int)bpl);fflush(stdout);
#endif
		png_destroy_write_struct(&png_ptr,&info_ptr);
		fclose(fp);
		return(0);
	...
	for(y=0,dbp=0;y<height;y++,dbp+=(width*3))
	{
		...
		for(x=0;x<width;x++)
		{
			...
			*(tmplbptr++)=SrcBuffer[tmpdbp++];
			*(tmplbptr++)=SrcBuffer[tmpdbp++];
			if(colortype==PNG_COLOR_TYPE_RGB_ALPHA) *(tmplbptr++)=SrcBuffer[tmpdbp++];
		}
		png_write_rows(png_ptr,(png_bytepp)lbfptrs,1);
	}
	png_write_flush(png_ptr);
	...
	{
#if VERBOSE>=1
		printf("\nPNG WRITE_RGB : Can not open for read-back '%s'.",FileName);fflush(stdout);
#endif
		return(1);
	}
...
int SavePNGFile(char *FileName, char *SrcBuffer, int Width, int Height, int NumOfComponent)
{
int NumClrs=0;
