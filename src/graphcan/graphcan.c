...
OldUpdateDriveMode(void)
{
	...
	RV = 255;
	GV = 255;
	BV = 0;			// Normal operation
	sprintf(Message, "Normal");
	break;
    case 0x80:
	RV = 255;
	GV = 0;
	BV = 0;			// Can not complete EV request
	sprintf(Message, "Denied");
	break;
    default:
	RV = 0;
	GV = 0;
	BV = 255;		// EV cancelled by override
    ...
    c = GetMyStringLength(Message, 0, 3);
    PutMyString(Message, (EV_XS + ((BAR_WIDTH - c) >> 1)), (((EV_YE - EV_YS) >> 1) + EV_YS - 8), 0,
		3);
